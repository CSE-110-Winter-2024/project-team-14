...
public interface GoalDao {
    ...
    @Query("SELECT * FROM goals WHERE type_of_goal = 'today' ORDER BY sort_order")
    List<GoalEntity> findAllTodayGoals();
    @Query("SELECT * FROM goals WHERE type_of_goal = 'tomorrow' ORDER BY sort_order")
    List<GoalEntity> findAllTomorrowGoals();
    @Query("SELECT * FROM goals WHERE type_of_goal != 'today' AND type_of_goal != 'tomorrow' ORDER BY sort_order")
    ...
    @Query("SELECT * FROM goals ORDER BY sort_order AND type_of_goal = 'today'")
    LiveData<List<GoalEntity>> findAllTodayGoalsAsLiveData();
    @Query("SELECT * FROM goals ORDER BY sort_order AND type_of_goal = 'tomorrow'")
    LiveData<List<GoalEntity>> findAllTomorrowGoalsAsLiveData();
    ...
    int countAll();
    @Query("SELECT COUNT(*) FROM goals WHERE type_of_goal = 'today'")
    int countTodayGoals();
    @Query("SELECT COUNT(*) FROM goals WHERE type_of_goal = 'tomorrow'")
    int countTomorrowGoals();
    @Query("SELECT COUNT(*) FROM goals  WHERE type_of_goal != 'today' AND type_of_goal != 'tomorrow'")
    int countRecurringGoals();
    @Query("SELECT MIN(sort_order) FROM goals  WHERE type_of_goal = 'today'")
    int getMinSortOrderToday();
    @Query("SELECT MAX(sort_order) FROM goals  WHERE type_of_goal = 'today'")
    int getMaxSortOrderToday();
    ...
    default int updateGoal(GoalEntity goal) {
        ...
        if (!goal.completed) {
            // append
            var maxSortOrder = getMaxSortOrderToday();
            var newGoal = new GoalEntity(goal.taskText, !goal.completed, maxSortOrder + 1000, goal.context, goal.typeOfGoal, goal.nextDate);
            newGoalId = Math.toIntExact(insert(newGoal));
        }
        else {
            // prepend
            shiftSortOrders(getMinSortOrderToday(), getMaxSortOrderToday(), 1);
            var newFlashcard = new GoalEntity(goal.taskText, !goal.completed, getMinSortOrderToday() - 1000, goal.context, goal.typeOfGoal, goal.nextDate);
