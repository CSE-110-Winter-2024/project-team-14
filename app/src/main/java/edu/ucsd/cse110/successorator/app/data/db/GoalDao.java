...
public interface GoalDao {
    ...
    default int updateGoal(GoalEntity goal) {
        ...
        if (!goal.completed) {
            ...
            var newGoal = new GoalEntity(goal.taskText, !goal.completed, maxSortOrder + 1000, goal.typeOfGoal, goal.nextDate);
            newGoalId = Math.toIntExact(insert(newGoal));
        }
        else {
            // prepend
            shiftSortOrders(getMinSortOrderOneTime(), getMaxSortOrderOneTime(), 1);
            var newFlashcard = new GoalEntity(goal.taskText, !goal.completed, getMinSortOrderOneTime() - 1000, goal.typeOfGoal, goal.nextDate);
